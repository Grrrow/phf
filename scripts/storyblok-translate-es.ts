...
function translateObj(obj: any): any {
  if (Array.isArray(obj)) {
    ...
    for (const key of Object.keys(obj)) {
      // Clean up previous wrong __i18n fields
      ...
      if (typeof val === 'string') {
        ...
        if (dict[trimmed]) {
          newObj[key] = val.replace(trimmed, dict[trimmed]);
        }
        ...
        if (val.type === 'doc') {
          ...
          if (translatedStr !== strVal) {
            newObj[key] = JSON.parse(translatedStr);
          }
...
async function main() {
  ...
  for (const story of exported) {
    ...
    console.log(`Translating story: ${story.name}...`);
    try {
      // Get the latest story without language suffix (gets English)
      const detailResp = await Storyblok.get(`spaces/${SPACE_ID}/stories/${story.id}`);
      const latestStory = detailResp.data.story;

      // Create a translated version of the content
      const translatedContent = translateObj(latestStory.content);

      // We MUST PUT with ?lang=es query param, so Storyblok stores it in the 'es' locale!
      await Storyblok.put(`spaces/${SPACE_ID}/stories/${story.id}?lang=es`, {
        story: {
          content: translatedContent
        }
      });
      console.log(`✅ Translated and uploaded ${story.name} (with ?lang=es)`);
    } catch (e: any) {
